    ...
    <table class="bordered">
        <thead class="text-center">
            <tr>
                ...
                <th rowspan=2 width=180px>{{ __('Officer Name') }}</th>
                <th colspan={{ $period->count() }}>{{ $start_date->translatedFormat('F') }}</th>
        ...
        <tbody>
            @foreach ($officers as $officer)
                <tr>
                    ...
                    <td>{{ $officer->name }}</td>
                    @foreach ($period as $date)
                        @if ($officer->schedules()->date($date->format('Y-m-d'))->first())
                            <td class="text-center">
                                {{ $officer->schedules()->date($date->format('Y-m-d'))->first()->attendance_record->attendanceStatusAbb ?? '-' }}
                            </td>
                        @else
            ...
            @endforeach
            <tr>
                <th class="text-end" colspan={{ $period->count() + 2 }}>
                    {{ __('Total') }}
                </th>
                @foreach (\App\Models\AttendanceRecord::STATUS as $status)
